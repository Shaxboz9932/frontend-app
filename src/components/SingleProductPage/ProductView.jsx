...
export default function ProductView({ className, reportHandler, id, restart }) {
...
for(let i = 1; i <= 5; i++){
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(`https://django-backend-8bva.onrender.com/products/${id}/`)
      try {
  ...
  const productFound = wishItems.find(item => Number(item.product.id) === Number(product.id))
  ...
  return (
    ...
    >
      <Helmet>
        <title>{`${product?.title} | SHopo`}</title>
      </Helmet>
      <div data-aos="fade-right" className="lg:w-1/2 xl:mr-[70px] lg:mr-[50px]">
        <div className="w-full">
          <div className="w-full h-[600px] border border-qgray-border flex justify-center items-center overflow-hidden relative mb-3">
            <img
              src={src ? `https://django-backend-8bva.onrender.com${src}` : `https://django-backend-8bva.onrender.com${product?.images?.[0]?.image}`}
              alt=""
          ...
          <div className="flex gap-2 flex-wrap">
            {
              product?.images?.map((img) => (
                ...
                >
                  <img
                    src={`https://django-backend-8bva.onrender.com${img.image}`}
                    alt=""
      ...
      <div className="flex-1">
        <div className="product-details w-full mt-10 lg:mt-0">
          ...
          <div data-aos="fade-up" className="flex space-x-2 items-center mb-7">
            ...
            <span className="text-2xl font-500 text-qred">${product.price * 0.5}</span>
          </div>
          ...
          >
              ...
              <button type="button">
                <span onClick={() => {
                  if(productFound) {
                    ...
                    addToWish(accessToken, product.id)
                  }
                }}>
                  <svg
                    width="24"
                  ...
                  >
                    <path
                      ...
                      stroke={productFound ? "yellow" : "#D5D5D5"}
                      fill={productFound ? "yellow" : ""}
                      strokeWidth="2"
                      strokeMiterlimit="10"
            ...
            <div className="flex-1 h-full">
              <button
                onClick={() => addToCart(accessToken, product.id, 1)}
                type="button"
