...
export default function ProductCardStyleOne({ datas, type }) {
  ...
  const addToWish = useWishListStore((state) => state.addToWish)
  ...
  const productFound = wishItems.find(item => Number(item.product.id) === Number(datas.id));
  ...
  return (
    ...
    >
      <div
        className="product-card-img w-full h-[300px] rounded-xl mt-8"
        style={{
          backgroundImage: datas?.images?.[0]?.image
            ? `url(https://django-backend-8bva.onrender.com${datas.images[0].image})`
            : 'none',
          backgroundSize: '95%',
          ...
          <>
            <div className="px-[30px] absolute left-0 top-3 w-full">
              <div className="progress-title flex justify-between ">
                <p className="text-xs mb-2 text-qblack font-400 leading-6">
                  Prodcuts Available
                </p>
              ...
              <div className="progress mb-4 w-full h-[5px] rounded-[22px] bg-primarygray relative overflow-hidden">
                <div
                  style={{
                    width: `${true ? 100 - available : 0}%`,
                  }}
                  className={`h-full absolute left-0 top-0 mb-4  ${
          ...
          <div className="product-type absolute right-[14px] top-[17px]">
            {/* <span
              className={`text-[9px] font-700 leading-none py-[6px] px-3 uppercase text-white rounded-full tracking-wider ${
                datas.title === "popular" ? "bg-[#19CC40]" : "bg-qyellow"
              }`}
            >
              {datas.product_type}
            </span> */}
      ...
      <div className="product-card-details px-[30px] pb-[30px] relative">
        ...
        <div className="absolute w-full h-10 px-[30px] left-0 top-40 group-hover:top-[85px] transition-all duration-300 ease-in-out">
          <button
            onClick={() => addToCart(accessToken, datas.id, 1)}
            type="button"
        ...
        <p className="price">
          <span className="main-price text-qgray line-through font-500 text-[14px]">
            {datas.price} so'm
          </span>
          <span className="offer-price text-qred font-500 text-[14px] ml-2">
            {datas.price - datas.price * 0.33} so'm
          </span>
        </p>
      </div>
      {/* quick-access-btns */}
      <div className="quick-access-btns flex flex-col space-y-2 absolute group-hover:right-4 -right-10 top-20  transition-all duration-300 ease-in-out">
        ...
        <span>
          <span onClick={() => {
            if (productFound) {
              deleteItem(accessToken, productFound.id)
            }else{
              addToWish(accessToken, datas.id)
            }
          }} className="w-10 h-10 flex justify-center items-center bg-primarygray rounded">
            <ThinLove productFound={productFound}/>
          </span>
        </span>
        <a href="#">
          <span className="w-10 h-10 flex justify-center items-center bg-primarygray rounded">
